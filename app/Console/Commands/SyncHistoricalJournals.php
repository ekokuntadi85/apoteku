...
class SyncHistoricalJournals extends Command
{
    ...
    private function syncTransactions()
    {
        ...
        // Use smaller chunk size due to heavy eager loading
        Transaction::with(['transactionDetails.product', 'transactionDetails.productUnit', 'transactionDetails.transactionDetailBatches.productBatch'])
            ->chunk(50, function ($transactions) use ($bar) {
    ...
    private function syncExpenses()
    {
        ...
        Expense::with('category')->chunk(100, function ($expenses) use ($bar) {
