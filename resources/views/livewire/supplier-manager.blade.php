<div class="container mx-auto p-4 dark:bg-gray-800 dark:text-gray-200">
	...
	<h1 class="text-3xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-rose-500">Daftar Supplier</h1>
	<div class="flex flex-col-reverse md:flex-row md:justify-between md:items-center mb-6 space-y-4 md:space-y-0">
        <div class="relative w-full md:w-1/3">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari supplier..." class="shadow appearance-none border rounded py-2 pl-10 pr-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline w-full dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600">
        </div>
		<button type="button" wire:click="createSupplier()" class="inline-flex items-center justify-center bg-gradient-to-r from-indigo-500 to-fuchsia-500 hover:from-indigo-600 hover:to-fuchsia-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-400 w-full md:w-auto md:ml-4">Tambah Supplier</button>
    </div>
			...
			<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
				...
				<tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
					...
					<tr class="hover:bg-indigo-50/60 dark:hover:bg-zinc-800/70 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-gray-200">{{ $supplier->id }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-gray-200">{{ $supplier->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-gray-200">{{ $supplier->phone }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-gray-200">{{ $supplier->address }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
							<button wire:click="edit({{ $supplier->id }})" class="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-1.5 px-3 rounded-full mr-2 dark:bg-emerald-600 dark:hover:bg-emerald-700">Edit</button>
							<button wire:click="delete({{ $supplier->id }})" onclick="confirm('Apakah Anda yakin ingin menghapus supplier ini?') || event.stopImmediatePropagation()" class="bg-rose-500 hover:bg-rose-600 text-white font-semibold py-1.5 px-3 rounded-full dark:bg-rose-600 dark:hover:bg-rose-700">Hapus</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
		...
		<div class="bg-white/80 dark:bg-gray-700/70 shadow-md rounded-xl p-4 border border-gray-200/70 dark:border-gray-600/60 backdrop-blur">
            <div class="flex justify-between items-start">
                ...
                <div class="flex space-x-2">
					<button wire:click="edit({{ $supplier->id }})" class="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-1 px-3 rounded-full text-xs dark:bg-emerald-600 dark:hover:bg-emerald-700">Edit</button>
					<button wire:click="delete({{ $supplier->id }})" onclick="confirm('Apakah Anda yakin ingin menghapus supplier ini?') || event.stopImmediatePropagation()" class="bg-rose-500 hover:bg-rose-600 text-white font-semibold py-1 px-3 rounded-full text-xs dark:bg-rose-600 dark:hover:bg-rose-700">Hapus</button>
                </div>
            </div>
        </div>
        @empty
        <p class="text-gray-600 dark:text-gray-400 text-center">Tidak ada supplier ditemukan.</p>
        @endforelse
    </div>
    ...
    <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" x-show="$wire.showModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800" @click.away="$wire.closeModal()">
            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white mb-4">{{ $isUpdateMode ? 'Edit Supplier' : 'Tambah Supplier Baru' }}</h3>
            <form wire:submit.prevent="save">
                <input type="hidden" wire:model="supplierId">
                <div class="mb-4">
                    <label for="name" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Nama Supplier:</label>
