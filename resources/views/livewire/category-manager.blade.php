<div class="container mx-auto p-4 dark:bg-gray-800 dark:text-gray-200">
	...
	<h1 class="text-3xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-rose-500">Daftar Kategori</h1>
				...
				<tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
					...
					<tr class="hover:bg-indigo-50/60 dark:hover:bg-zinc-800/70 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
							<button wire:click="edit({{ $category->id }})" class="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-1.5 px-3 rounded-full mr-2 dark:bg-emerald-600 dark:hover:bg-emerald-700">Edit</button>
							<button wire:click="delete({{ $category->id }})" onclick="confirm('Apakah Anda yakin ingin menghapus kategori ini?') || event.stopImmediatePropagation()" class="bg-rose-500 hover:bg-rose-600 text-white font-semibold py-1.5 px-3 rounded-full dark:bg-rose-600 dark:hover:bg-rose-700">Hapus</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
		...
		<div class="bg-white/80 dark:bg-gray-700/70 shadow-md rounded-xl p-4 border border-gray-200/70 dark:border-gray-600/60 backdrop-blur">
            <div class="flex justify-between items-start">
                ...
                <div class="flex space-x-2">
					...
					<button wire:click="delete({{ $category->id }})" onclick="confirm('Apakah Anda yakin ingin menghapus kategori ini?') || event.stopImmediatePropagation()" class="bg-rose-500 hover:bg-rose-600 text-white font-semibold py-1 px-3 rounded-full text-xs dark:bg-rose-600 dark:hover:bg-rose-700">Hapus</button>
                </div>
            </div>
        </div>
        @empty
