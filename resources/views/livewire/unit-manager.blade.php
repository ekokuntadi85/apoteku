<div class="container mx-auto p-4 dark:bg-gray-800 dark:text-gray-200">
			...
			<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
				<thead class="bg-gradient-to-r from-indigo-50 to-fuchsia-50 dark:from-zinc-800 dark:to-zinc-800">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">Nama Singkat</th>
    ...
    <div class="block md:hidden space-y-4">
        ...
        @empty
        <p class="text-gray-600 dark:text-gray-400 text-center">Tidak ada unit ditemukan.</p>
        @endforelse
    </div>
    ...
    <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" x-show="$wire.showModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800" @click.away="$wire.closeModal()">
            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white mb-4">{{ $isUpdateMode ? 'Edit Unit' : 'Tambah Unit Baru' }}</h3>
            <form wire:submit.prevent="save">
                <input type="hidden" wire:model="unitId">
                <div class="mb-4">
                    <label for="name" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Nama Unit:</label>
                    <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600" id="name" placeholder="Masukkan Nama Unit" wire:model="name">
                    @error('name') <span class="text-red-500 text-xs italic">{{ $message }}</span>@enderror
                </div>
                <div class="mb-6">
                    <label for="short_name" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Nama Singkat:</label>
                    <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600" id="short_name" placeholder="Masukkan Nama Singkat (misal: pcs, kg)" wire:model="short_name">
